...
def plot_scatter_with_errorbars(data, custom_labels, numb, legend_prefix=''):
    ...
    markers = ['s', 'o', 'D', '^', 'p', 'h', 'd', 'x']
    colors = ['black', 'blue', 'red', 'green', 'purple', 'orange', 'navy']
    marker_size = 50  # 마커 크기 조정
    edge_width = 0.5    # 엣지 두께 조정
    # 라벨 색인을 위한 변수
    ax.grid(True)
    # 임계값 설정
    threshold = 1e-10
    ...
    for i in range(0, len(y_cols), 3):
        ...
        # scatter 그리기
        ax.plot(x_col, cleandata[scatter_col], color='black', linewidth=0.5, alpha=1)
        ax.scatter(x_col, cleandata[scatter_col], marker=markers[i // 3], color='None', edgecolor='black', s=marker_size, linewidths=edge_width, label=f"{legend_prefix}{custom_labels[i // 3]}", alpha=1)
